...
func (impl *HttpTimeoutExecutor) start(ctx context.Context, url string, t int) *spec.Response {
	...
	if err != nil {
		log.Errorf(ctx, "http NewRequestWithContext creation failed:", err.Error())
	...
	if err != nil {
		log.Errorf(ctx, "Request failed:", err.Error())
		return spec.ResponseFailWithFlags(spec.ActionNotSupport, "Request-failed", t)
	}
